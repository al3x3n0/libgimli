...
// Parser for DWARF 5 .debug_loclists section
class LocListsParser {
public:
    LocListsParser(const std::vector<uint8_t>& debug_loclists,
                   const std::vector<uint8_t>& debug_addr = {});

    // Parse header at the given offset, returns header and advances offset past it
    std::optional<LocListsHeader> parseHeader(uint64_t& offset) const;

    // Parse a location list starting at the given offset
    // base_address is the current base address for offset_pair entries
    // Returns list of location entries (with resolved addresses and expressions)
    std::vector<LocListEntry> parseLocationList(uint64_t offset,
                                                 uint64_t base_address = 0,
                                                 uint8_t address_size = 8,
                                                 uint64_t end_offset = 0,
                                                 uint8_t segment_selector_size = 0) const;

    // Resolve DW_FORM_loclistx index to actual offset
    // header_offset is the start of the loclists contribution for this CU
    // loclists_base is DW_AT_loclists_base value
    uint64_t resolveLocListx(uint64_t index,
    ...
    bool empty() const { return debug_loclists_.empty(); }
    size_t size() const { return debug_loclists_.size(); }
