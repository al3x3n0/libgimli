...
const std::vector<SupportMatrixRow>& getSupportMatrixRows() {
    static const std::vector<SupportMatrixRow> kRows = {
        ...
        {"expr", "GNU extensions", "partial",
         "major GNU ops supported (including parameter_ref, implicit_pointer referent preservation, GNU_uninit taint propagation, encoded_addr with absptr/pcrel/textrel/datarel/funcrel/aligned handling plus unknown-format absptr fallback, typed predecessors, and *_index utility decoding); unsupported-op diagnostics include raw opcode bytes/vendor classification plus structured unsupported-op metadata; unknown vendor ops are rejected"},
