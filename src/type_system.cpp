...
std::string PrimitiveType::getDescription() const {
    ...
    if (binary_scale_ != 0) {
        ss << " [binary_scale=" << binary_scale_ << "]";
    }
    if (decimal_scale_ != 0) {
        ss << " [decimal_scale=" << decimal_scale_ << "]";
    }
    if (decimal_sign_ != 0) {
        ss << " [decimal_sign=" << decimal_sign_ << "]";
    }
    if (digit_count_ != 0) {
...
std::string FunctionType::getDescription() const {
    ...
    if (priority_ != 0) {
        description += " [priority=" + std::to_string(priority_) + "]";
    }
    ...
    if (!description_text_.empty()) {
        description += " [description=" + description_text_ + "]";
    }
    return description;
}
...
std::shared_ptr<Type> TypeSystem::createPrimitiveType(PrimitiveType::Kind kind, uint64_t size,
    ...
    auto type = std::make_shared<PrimitiveType>(kind, size, name, endianity, address_class,
                                                binary_scale, decimal_scale, decimal_sign, digit_count,
                                                picture_string,
...
std::shared_ptr<Type> TypeSystem::createFunctionType(std::shared_ptr<Type> return_type,
    ...
    auto type = std::make_shared<FunctionType>(return_type, parameter_types, is_variadic, is_prototyped,
                                               inline_code, priority, calling_convention, is_external, is_declaration, is_explicit,
                                               is_elemental, is_pure, is_recursive, is_main_subprogram,
                                               is_const_expr, visibility, linkage_name, description_text);
...
std::shared_ptr<Type> TypeSystem::resolvePrimitiveType(std::shared_ptr<DIE> die) {
    ...
    if (binary_scale_attr && binary_scale_attr->getType() == AttributeValueType::UNSIGNED) {
        binary_scale = std::static_pointer_cast<UnsignedAttributeValue>(binary_scale_attr)->getValue();
    }
    auto decimal_scale_attr = die->getAttribute(DwarfAttribute::DW_AT_decimal_scale);
    if (decimal_scale_attr && decimal_scale_attr->getType() == AttributeValueType::UNSIGNED) {
        decimal_scale = std::static_pointer_cast<UnsignedAttributeValue>(decimal_scale_attr)->getValue();
    }
    auto decimal_sign_attr = die->getAttribute(DwarfAttribute::DW_AT_decimal_sign);
    if (decimal_sign_attr && decimal_sign_attr->getType() == AttributeValueType::UNSIGNED) {
        decimal_sign = std::static_pointer_cast<UnsignedAttributeValue>(decimal_sign_attr)->getValue();
    }
    ...
    if (threads_scaled_attr && threads_scaled_attr->getType() == AttributeValueType::FLAG) {
        threads_scaled = std::static_pointer_cast<FlagAttributeValue>(threads_scaled_attr)->getValue();
    }

    return createPrimitiveType(kind, size, name, endianity, address_class, binary_scale, decimal_scale,
                               decimal_sign, digit_count, picture_string, is_small, threads_scaled);
}
...
std::shared_ptr<Type> TypeSystem::resolveEnumType(std::shared_ptr<DIE> die) {
    ...
    auto enum_type = std::dynamic_pointer_cast<EnumType>(
        createEnumType(name, resolved_underlying, is_scoped, visibility, is_declaration));
...
std::shared_ptr<Type> TypeSystem::resolveFunctionType(std::shared_ptr<DIE> die) {
    ...
    if (priority_attr && priority_attr->getType() == AttributeValueType::UNSIGNED) {
        priority = std::static_pointer_cast<UnsignedAttributeValue>(priority_attr)->getValue();
    }
    ...
    if (external_attr && external_attr->getType() == AttributeValueType::FLAG) {
        is_external = std::static_pointer_cast<FlagAttributeValue>(external_attr)->getValue();
    }
    auto declaration_attr = die->getAttribute(DwarfAttribute::DW_AT_declaration);
    ...
    if (description_attr && description_attr->getType() == AttributeValueType::STRING) {
        description_text = std::static_pointer_cast<StringAttributeValue>(description_attr)->getValue();
    }
    ...
    return createFunctionType(resolved_return, parameters, is_variadic, is_prototyped, inline_code, priority, calling_convention,
                              is_external, is_declaration, is_explicit, is_elemental, is_pure, is_recursive,
                              is_main_subprogram, is_const_expr, visibility, linkage_name, description_text);
}
